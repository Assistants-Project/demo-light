...
async fn change_light(light: AnyPin<'static>, notifier: Notifier<bool>) {
    ...
    loop {
        ...
        match light_input {
            ...
            LightInput::Toggle => {
                if LIGHT_MODE.load(Ordering::Relaxed) == LightMode::Manual as u8
                && TOGGLE_CONTROLLER.load(Ordering::Relaxed)
                {
                    toggle_light(&mut light);
                }
            }
...
async fn toggle_task() {
    loop {
        // Wait until toggle is enabled.
        while !TOGGLE_CONTROLLER.load(Ordering::Relaxed) {
            Timer::after_millis(50).await;
        }

        // If we are not in manual mode, disable toggle.
        if LIGHT_MODE.load(Ordering::Relaxed) != LightMode::Manual as u8 {
            TOGGLE_CONTROLLER.store(false, Ordering::Relaxed);
            continue;
        }

        // Every 5 seconds, send a single Toggle.
        Timer::after_secs(5).await;

        if TOGGLE_CONTROLLER.load(Ordering::Relaxed)
            && LIGHT_MODE.load(Ordering::Relaxed) == LightMode::Manual as u8
        {
...
#[inline]
fn ensure_manual_mode() -> Result<(), ErrorResponse> {
    if LIGHT_MODE.load(Ordering::Relaxed) == LightMode::Manual as u8 {
        Ok(())
    } else {
        Err(ErrorResponse::invalid_data(
            "Route available only in MANUAL mode",
        ))
    }
}
...
async fn main(spawner: Spawner) {
    ...
    spawner
        .spawn(toggle_task())
        .expect("Impossible to spawn the task to control the toggle route");
    spawner
