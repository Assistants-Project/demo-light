...
- `light/manual` route enables manual mode, allowing control via the physical switch or the `/on`, `/off` and `/toggle` routes
- `light/motion-detection` route enables motion-based mode, where the light turns on/off based on the AM312 PIR motion sensor
- `light/ambient-light` route enables ambient-light mode, where the light turns on/off depending on the light level measured by the BH1750 sensor
